...
def give_me_url (category):
    ...
    while j < how_many_shop_is_there(category):
        ...
        j = j + 1
    return page_urls
def give_me_ecommerce_of_each_urls(page_url):
    try:
        with open('ecommerce.csv','rt')as f:
            data = csv.reader(f)
            for row in data:
                if row[0].find(page_url)>-1:
                    return row[1]
    except Exception:
        return False
all_of_urls =  give_me_all_of_the_urls()
categorys = read_file('category.txt')
i= 0
for category in categorys :
    print(len(categorys) - i)
    try :
        category = category.split('\n')[0]
    except Exception :
        pass
    for page_url in give_me_url(category):
        try :
            page_url = page_url.split('\n')[0]
        except Exception :
            pass    
        if page_url in all_of_urls:
            ecommerce = give_me_ecommerce_of_each_urls(page_url)
            add_array_in_csv_file(category + '.csv' ,ecommerce , page_url)
    i = i + 1
